...
def _make_plan(
    ...
    reason = (
        f"Clean Blend: {v_type}→{i_type} anchors @ {instr_anchor:.2f}s, "
...
def generate_candidates(
    ...
    *,
    ...
    for tm in tempo_mults:
        # Limit cross product — sample evenly
...
def plan_summary_for_ui(plan: RemixPlan) -> dict:
    ...
    return {
        ...
        "tempo_label": (
            f"vocal {plan.vocal_bpm_effective:.1f} → beat {plan.target_bpm:.1f} BPM "
...
def _key_label(plan: RemixPlan) -> str:
    ...
    if plan.harmonic_compatibility == "weak":
        return "low key confidence — no shift"
    if abs(shift) >= 0.5:
